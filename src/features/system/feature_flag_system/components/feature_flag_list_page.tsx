    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>{t("new_flag_description")}</DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-2">
            ...
            <Input
              ...
              required
            />
            <p className="text-muted-foreground text-xs">{t("key_description")}</p>
          </div>
          <Separator />
  ...
  const columns = React.useMemo<ColumnDef<FeatureFlagRow>[]>(
    () => [
          ...
          <div className="flex items-center gap-2">
            <Switch
              ...
              onCheckedChange={(checked) =>
                optimisticToggle({
                  query_key: getQueryKey(
                    trpc.featureFlags.list,
                    { page, limit: per_page, search: search || undefined },
                    "query",
                  ),
                  updater: (old: { items: FeatureFlagRow[] } | undefined) =>
                    old
                      ? {
                          ...old,
                          items: old.items.map((f) =>
                            f.id === row.original.id ? { ...f, enabled: checked } : f,
                          ),
                        }
                      : { items: [] as FeatureFlagRow[] },
      ...
      },
      {
        ...
        header: ({ column }) => (
          <DataTableColumnHeader column={column} label={t("description_column")} />
        ),
        cell: ({ row }) => (
      ...
      },
      {
        ...
        cell: ({ row }) =>
          row.original.updated_at ? formatDate(row.original.updated_at, { month: "short" }) : "—",
      },
      {
        id: "actions",
        cell: ({ row }) => (
                        ...
                        ? {
                            ...
                            items: old.items.map((f) =>
                              f.id === row.original.id
                                ? { ...f, enabled: !row.original.enabled }
                                : f,
                            ),
                          }
                        : { items: [] as FeatureFlagRow[] },
                    mutate: () =>
                      toggle.mutateAsync({ id: row.original.id, enabled: !row.original.enabled }),
                    success_key: "toggle_updated",
                    error_key: "action_failed",
                  })
                }
    ...
    [t, optimisticToggle, page, per_page, search, toggle],
  );
