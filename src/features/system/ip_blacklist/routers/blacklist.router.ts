...
} from "@/features/system/ip_blacklist/validators/blacklist.validator";
...
export const blacklist_router = create_trpc_router({
  ...
  list: permission_procedure(PERMISSIONS.blacklist_view)
    .input(list_blacklist_schema)
    .query(async ({ input }) => {
      return ip_blacklist_service.list(input);
    }),

  get_by_id: permission_procedure(PERMISSIONS.blacklist_view)
    .input(z.object({ id: z.string() }))
    .query(async ({ input }) => {
      return ip_blacklist_service.get_by_id(input.id);
    }),

  add: permission_procedure(PERMISSIONS.blacklist_create)
    .input(add_to_blacklist_schema)
  ...
  update: permission_procedure(PERMISSIONS.blacklist_update)
    .input(z.object({ id: z.string(), data: update_blacklist_schema }))
    .mutation(async ({ input }) => {
      return ip_blacklist_service.update(input.id, input.data);
    }),

  remove: permission_procedure(PERMISSIONS.blacklist_delete)
    .input(z.object({ id: z.string() }))
    .mutation(async ({ input }) => {
  ...
  toggle: permission_procedure(PERMISSIONS.blacklist_update)
    .input(z.object({ id: z.string() }))
    .mutation(async ({ input }) => {
