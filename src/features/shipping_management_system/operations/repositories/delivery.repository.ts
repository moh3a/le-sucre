...
export class DeliveryRepository {
  ...
  async get_attempts_by_shipment(shipment_id: string) {
    return db.select().from(delivery_attempts).where(eq(delivery_attempts.shipment_id, shipment_id)).orderBy(desc(delivery_attempts.attempt_number));
  }

  async get_attempts_by_order(order_id: string) {
    return db.select().from(delivery_attempts).where(eq(delivery_attempts.order_id, order_id)).orderBy(desc(delivery_attempts.attempted_at));
  }

  async get_latest_attempt(shipment_id: string) {
    return db.select().from(delivery_attempts).where(eq(delivery_attempts.shipment_id, shipment_id)).orderBy(desc(delivery_attempts.attempt_number)).limit(1).then((r) => r[0] ?? null);
  }

  async count_failed_deliveries(since?: string) {
    const where = since ? and(eq(delivery_attempts.status, "failed"), sql`${delivery_attempts.attempted_at} >= ${since}`) : eq(delivery_attempts.status, "failed");
  ...
  async get_stats() {
    const [[{ failed }], [{ successful }], [{ pending }], [{ rto }], [{ today_failed }]] = await Promise.all([
      ...
      db.select({ today_failed: count() }).from(delivery_attempts).where(and(eq(delivery_attempts.status, "failed"), sql`DATE(${delivery_attempts.attempted_at}) = CURDATE()`)),
    ]);
    return { total_failed: Number(failed ?? 0), total_successful: Number(successful ?? 0), pending_retries: Number(pending ?? 0), total_rto: Number(rto ?? 0), today_failed: Number(today_failed ?? 0) };
  }
}
