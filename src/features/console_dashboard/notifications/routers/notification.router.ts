...
export const notification_router = create_trpc_router({
  notificationList: protected_procedure
    .input(
      z.object({
  ...
  notificationMarkAsRead: protected_procedure
    .input(z.object({ id: z.string() }))
    .mutation(({ ctx, input }) =>
      notification_service.mark_as_read(input.id, ctx.session!.user.id),
    ),

  notificationMarkAllAsRead: protected_procedure.mutation(({ ctx }) =>
    notification_service.mark_all_as_read(ctx.session!.user.id),
  ),

  notificationCountUnread: protected_procedure.query(({ ctx }) =>
    notification_service.count_unread(ctx.session!.user.id),
  ),
});
