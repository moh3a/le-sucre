...
export function NotificationsPageClient() {
  ...
  const { data: result, refetch } = trpc.operations.notificationList.useQuery({
    page,
    limit: 20,
    unread_only: false,
  });
      ...
      <div className="rounded-lg border">
        <div className="divide-y">
          {notifications.map((n: NotificationRow) => (
            ...
            >
              <div
                className={`mt-1.5 h-2 w-2 shrink-0 rounded-full ${n.is_read ? "bg-transparent" : "bg-blue-500"}`}
              />
          ...
          {notifications.length === 0 && (
            <p className="p-8 text-center text-sm text-gray-400">{t("no_notifications")}</p>
          )}
        </div>
      </div>
      ...
      {meta && (
        <div className="flex justify-center gap-2">
          ...
          <span className="px-3 py-1 text-sm text-gray-500">
            {t("page")} {page} {t("of")} {meta.total_pages}
          </span>
          <button
