...
import { ChevronLeft, ChevronRight, Phone, Mail, MessageCircle, Plus } from "lucide-react";
...
export function CustomerContactsTab({ user_id }: CustomerContactsTabProps) {
  ...
  const { data, isLoading, refetch } = trpc.operations.customerGetContacts.useQuery({
    user_id,
    page,
    limit: 20,
  });
              ...
              return (
                <div key={c.id} className="flex items-start gap-3 rounded-md border p-3 text-sm">
                  ...
                  <div className="flex-1">
                    ...
                    {c.subject && (
                      <p className="text-muted-foreground mt-0.5 text-xs">{c.subject}</p>
                    )}
