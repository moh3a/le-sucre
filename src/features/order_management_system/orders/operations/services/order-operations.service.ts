...
export class OrderOperationsService {
  async assign_operator(input: { order_id: string; operator_id: string; actor_user_id: string; note?: string }) {
    ...
    await repo.insert_assignment({
      id: generate_id(), order_id: input.order_id, assignment_type: "operator",
      from_user_id: old_operator_id ?? null, to_user_id: input.operator_id,
      assigned_by_user_id: input.actor_user_id, note: input.note ?? null,
    });
  ...
  async escalate(input: { order_id: string; reason: string; description?: string; priority?: string; assigned_to_user_id?: string; escalated_by_user_id: string }) {
    const order = await order_repository.find_by_id(input.order_id);
    if (!order) throw_error(OPERATIONS_ERROR.NOT_FOUND);
    const id = await repo.insert_escalation({
      id: generate_id(), order_id: input.order_id, escalated_by_user_id: input.escalated_by_user_id,
      assigned_to_user_id: input.assigned_to_user_id ?? null, reason: input.reason,
      description: input.description ?? null, priority: input.priority ?? "normal", status: "open",
    });
    await order_repository.insert_status_event({
      id: generate_id(), order_id: input.order_id, from_status: order.status, to_status: order.status,
  ...
  async resolve_escalation(input: { id: string; resolution: string; resolved_by_user_id: string; status?: string }) {
    ...
    if (escalation.status === "resolved" || escalation.status === "dismissed") throw_error(OPERATIONS_ERROR.INVALID_STATUS);
    await repo.update_escalation(input.id, {
      status: input.status ?? "resolved", resolution: input.resolution,
      resolved_by_user_id: input.resolved_by_user_id, resolved_at: new Date().toISOString(),
    });
  }

  async get_escalations(order_id: string) { return repo.get_escalations_by_order(order_id); }
  ...
  async place_on_hold(input: { order_id: string; reason: string; description?: string; held_by_user_id: string }) {
    ...
    await repo.insert_hold({
      id: generate_id(), order_id: input.order_id, reason: input.reason,
      description: input.description ?? null, held_by_user_id: input.held_by_user_id, is_active: true,
    });
    await order_repository.insert_status_event({
  ...
  async release_hold(input: { hold_id: string; released_by_user_id: string; reason?: string }) {
    ...
    const hold_record = await db.select().from(order_holds).where(eq(order_holds.id, input.hold_id)).limit(1).then((r) => r[0] ?? null);
    ...
    await repo.release_hold(input.hold_id, {
      is_active: false, released_by_user_id: input.released_by_user_id,
      released_at: new Date().toISOString(), released_reason: input.reason ?? null,
    });
    await order_repository.insert_status_event({
  ...
  async request_cancellation(input: { order_id: string; reason: string; description?: string; requested_by_user_id: string }) {
    const order = await order_repository.find_by_id(input.order_id);
    if (!order) throw_error(OPERATIONS_ERROR.NOT_FOUND);
    if (order.status === "cancelled" || order.status === "refunded") throw_error(OPERATIONS_ERROR.INVALID_STATUS);
    ...
    const id = await repo.insert_cancellation_request({
      id: generate_id(), order_id: input.order_id, requested_by_user_id: input.requested_by_user_id,
      reason: input.reason, description: input.description ?? null, status: "pending",
    });
    await order_repository.insert_status_event({
      id: generate_id(), order_id: input.order_id, from_status: order.status, to_status: order.status,
  ...
  async review_cancellation(input: { cancellation_request_id: string; status: "approved" | "rejected"; review_note?: string; reviewed_by_user_id: string; refund_amount?: number }) {
    ...
    await repo.update_cancellation_request(input.cancellation_request_id, {
      status: input.status, reviewed_by_user_id: input.reviewed_by_user_id,
      review_note: input.review_note ?? null, reviewed_at: new Date().toISOString(),
      ...(input.refund_amount !== undefined ? { refund_amount: String(input.refund_amount) } : {}),
    });
    if (input.status === "approved") {
      await order_repository.update_order_status(request.order_id, "cancelled", { cancelled_at: new Date().toISOString() });
      await order_repository.insert_status_event({
        id: generate_id(), order_id: request.order_id, from_status: "pending_cancellation", to_status: "cancelled",
        actor_user_id: input.reviewed_by_user_id, note: `Annulation approuvée: ${input.review_note ?? ""}`,
      });
    }
  }

  async get_cancellation_requests(order_id: string) { return repo.get_cancellation_requests_by_order(order_id); }
  ...
  async add_comment(input: { order_id: string; content: string; is_private?: boolean; author_user_id: string }) {
    ...
    const id = await repo.insert_comment({
      id: generate_id(), order_id: input.order_id, author_user_id: input.author_user_id,
  ...
  async get_timeline(order_id: string) {
    ...
    const timeline = [
      ...status_events.map((e) => ({ type: "status_event" as const, id: e.id, date: e.created_at, actor_user_id: e.actor_user_id, description: e.note ?? `${e.from_status ?? "created"} → ${e.to_status}`, metadata: { from_status: e.from_status, to_status: e.to_status } })),
      ...assignments.map((a) => ({ type: "assignment" as const, id: a.id, date: a.created_at, actor_user_id: a.assigned_by_user_id, description: a.note ?? `Assigné: ${a.assignment_type}`, metadata: { assignment_type: a.assignment_type, from_user_id: a.from_user_id, to_user_id: a.to_user_id } })),
      ...holds.map((h) => ({ type: h.is_active ? ("hold" as const) : ("hold_released" as const), id: h.id, date: h.created_at, actor_user_id: h.held_by_user_id, description: `Mise en attente: ${h.reason}`, metadata: { reason: h.reason } })),
      ...escalations.map((e) => ({ type: "escalation" as const, id: e.id, date: e.created_at, actor_user_id: e.escalated_by_user_id, description: `Escalade: ${e.reason}`, metadata: { status: e.status, priority: e.priority } })),
      ...comments.map((c) => ({ type: "comment" as const, id: c.id, date: c.created_at, actor_user_id: c.author_user_id, description: c.content.slice(0, 200), metadata: { is_private: c.is_private } })),
      ...cancellations.map((c) => ({ type: "cancellation_request" as const, id: c.id, date: c.created_at, actor_user_id: c.requested_by_user_id, description: `Demande d'annulation: ${c.reason} (${c.status})`, metadata: { status: c.status, reason: c.reason } })),
    ];
