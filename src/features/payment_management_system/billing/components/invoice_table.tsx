...
export function InvoiceTable() {
  ...
  const STATUS_LABEL: Record<string, string> = React.useMemo(
    () => ({
      paid: t("status_paid"),
  ...
  const TYPE_LABEL: Record<string, string> = React.useMemo(
    () => ({
  ...
  const columns = React.useMemo<ColumnDef<InvoiceRow>[]>(
    () => [
      ...
      },
      {
        id: "customer",
        header: ({ column }) => (
          <DataTableColumnHeader column={column} label={t("client_column")} />
        ),
        cell: ({ row }) => (
    ...
    [STATUS_LABEL, TYPE_LABEL, delete_mutation, handleDownload, t],
  );
