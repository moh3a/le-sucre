...
export function WarehousesClient() {
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      {showForm && (
        <form onSubmit={handleCreate} className="space-y-3 rounded-lg border bg-white p-4">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="mb-1 block text-sm font-medium">{tc("name")}</label>
              <input
                value={name}
            ...
            <div>
              <label className="mb-1 block text-sm font-medium">{tc("slug")}</label>
              <input
                value={slug}
        ...
        <div className="divide-y">
          {warehouses?.map((w: WarehouseOption) => (
            <div key={w.id} className="flex items-center justify-between px-4 py-3">
              ...
              <span
                className={`rounded-full px-2 py-0.5 text-xs ${w.is_active === false ? "bg-gray-100 text-gray-500" : "bg-green-100 text-green-700"}`}
              >
                {w.is_active === false ? tc("inactive") : tc("active")}
              </span>
            </div>
          ...
          {(!warehouses || warehouses.length === 0) && (
            <p className="p-4 text-sm text-gray-400">{tc("no_results")}</p>
          )}
        </div>
      </div>
    </div>
