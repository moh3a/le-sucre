...
export function InventoryAdjustmentsTable() {
    ...
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          return (
            <span className={`font-mono text-sm ${delta > 0 ? "text-green-600" : "text-red-600"}`}>
              {delta > 0 ? `+${delta}` : delta}
            </span>
          );
        },
              ...
              <DropdownMenuContent align="end">
                ...
                {r.status === "pending" && (
                  <>
                    <DropdownMenuItem
                      onClick={() =>
                        reviewMutation.mutate({ id: r.id, status: "approved" })
                      }
                    >
  ...
  const { data, isLoading } = trpc.operations.inventoryListAdjustmentRequests.useQuery({
    page,
    limit: per_page,
    status: status || undefined,
  });

  const items = (data?.items ?? []) as unknown as AdjustmentRow[];
  ...
  const { table } = useDataTable({
    ...
    queryKeys: { page: "iaPage", perPage: "iaPerPage", sort: "iaSort" },
    getRowId: (row) => row.id,
      ...
      {table.getFilteredSelectedRowModel().rows.length > 0 && (
        <div className="flex items-center gap-2 border-t p-2">
          ...
          <Button variant="ghost" size="sm" asChild>
            <a
              href={`/api/admin/inventory-adjustments/export?${new URLSearchParams({
                ...(status ? { status } : {}),
              })}`}
