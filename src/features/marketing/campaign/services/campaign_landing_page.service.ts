...
import {
  campaign_recommendation_service,
  type RecommendationStrategy,
} from "./campaign_recommendation.service";
...
export class CampaignLandingPageService {
  async get_landing_page(
    ...
    const sections = await db
      ...
      .where(
        and(eq(campaign_sections.campaign_id, campaign.id), eq(campaign_sections.is_active, true)),
      )
  ...
  async get_landing_pages_for_storefront() // locale: string = "fr",
  : Promise<
    Array<{
      slug: string;
      name: string;
