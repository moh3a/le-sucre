...
export class CampaignTargetingService {
  ...
  async evaluate_behavior_rules(
    ...
    for (const rule of rules) {
      ...
      const result = await this._evaluate_behavior_rule(
        rule.behavior_rule,
  ...
  async find_matching_campaigns(
    ctx: TargetingContext,
  ): Promise<Array<{ campaign_id: string; score: number }>> {
  ...
  async get_personalized_campaigns(
    user_id: string,
    locale: string,
    country?: string,
  ): Promise<string[]> {
  ...
  private async _evaluate_behavior_rule(
    ...
    switch (rule) {
      case "viewed_product": {
        if (!ctx.viewed_product_ids?.length || !config.product_ids)
      ...
      case "purchased_category": {
        if (!ctx.purchased_category_ids?.length || !config.category_ids)
          return { matched: false, score: 0, rule_name: rule };
