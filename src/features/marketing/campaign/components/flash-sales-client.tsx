...
export function FlashSalesClient() {
  ...
  const [page, setPage] = useQueryState("fsPage", parseAsInteger.withDefault(1));
  const [per_page] = useQueryState("fsPerPage", parseAsInteger.withDefault(10));
  const [search, setSearch] = useQueryState("fsSearch", parseAsString);
  const [status, setStatus] = useQueryState("fsStatus", parseAsString);
  ...
  const { data: stats, isLoading: statsLoading } = trpc.campaigns.flashSaleStats.useQuery();
  const utils = trpc.useUtils();

  const setStatusMutation = trpc.campaigns.setStatus.useMutation({
    onSuccess: () => {
      utils.campaigns.adminList.invalidate();
      utils.campaigns.flashSaleStats.invalidate();
    },
    onError: (err) => {
      toast.error(err.message || t("flash_sales_update_error"));
    },
  });

  const columns = React.useMemo<ColumnDef<FlashSaleRow>[]>(
    () => [
      {
        id: "select",
        enableSorting: false,
  ...
  const items = (data?.items ?? []) as FlashSaleRow[];
  ...
  const { table } = useDataTable({
    ...
    queryKeys: { page: "fsPage", perPage: "fsPerPage", sort: "fsSort" },
    getRowId: (row) => row.id,
  });
  ...
  return (
    ...
    >
      <ConsolePageShell
        title={t("flash_sales")}
        subtitle={t("flash_sales_subtitle")}
        actions={
          <Button asChild>
            <Link href="/console/campaigns/new">
              <Plus className="mr-2 size-4" />
              {t("flash_sales_create")}
            </Link>
          </Button>
        }
        stats={
          <StatsGrid
            ...
            items={[
              { label: t("flash_sales_total"), value: stats?.total ?? 0, icon: Layers, color: "info" },
              {
                label: t("flash_sales_active"),
                value: stats?.active ?? 0,
                icon: Flame,
                color: "success",
              },
              {
                label: t("flash_sales_upcoming"),
                value: stats?.scheduled ?? 0,
                icon: CalendarClock,
                color: "warning",
              },
              {
                label: t("flash_sales_ended"),
                value: stats?.ended ?? 0,
                icon: Square,
                color: "default",
              },
