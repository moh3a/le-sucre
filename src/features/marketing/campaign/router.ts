...
} from "./services/campaign_automation.service";
import {
  list_campaigns_dto,
  create_campaign_dto,
  update_campaign_dto,
...
export const campaign_router = create_trpc_router({
  ...
  abTestVariant: public_procedure
    ...
    .query(({ input }) =>
      campaign_ab_test_service.get_variant_for_user(
        input.test_group,
        input.user_id,
        input.session_seed,
      ),
  ...
  abTestReport: permission_procedure(PERMISSIONS.campaigns_read)
    ...
    .query(({ input }) =>
      campaign_ab_test_service.get_ab_test_report(input.test_group, input.days),
    ),

  abTestGroups: permission_procedure(PERMISSIONS.campaigns_read).query(() =>
    campaign_service.list_ab_test_groups(),
  ),
  ...
  automationRules: permission_procedure(PERMISSIONS.campaigns_read).query(() =>
    campaign_automation_service.list_rules(),
  ),
  ...
  createAutomationRule: permission_procedure(PERMISSIONS.campaigns_write)
    ...
    .mutation(({ input }) => campaign_automation_service.create_rule(input)),
