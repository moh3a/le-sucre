...
export class CustomerRelationsService {
  ...
  async log_contact(input: {
    ...
    const id = await repo.insert_contact({
      ...
      handled_by_user_id: input.handled_by_user_id ?? null,
      metadata: {},
    });

    void audit_service.log({
      actor_user_id: input.handled_by_user_id,
      action: "customer.contact.log",
      resource_type: "user_id",
      resource_id: input.user_id ?? undefined,
      metadata: { contact_type: input.contact_type, direction: input.direction },
    });

    return id;
  }

  async get_contacts(user_id: string, page = 1, limit = 20) {
    return repo.get_contacts_by_user(user_id, page, limit);
  }

  async get_contacts_by_order(order_id: string) {
    return repo.get_contacts_by_order(order_id);
  }

  // ─── NOTES ────────────────────────────────────
  async add_note(input: {
    user_id: string;
    note_type: string;
    content: string;
  ...
  async get_notes(user_id: string, note_type?: string) {
    return repo.get_notes_by_user(user_id, note_type);
  }

  async toggle_pin(note_id: string, is_pinned: boolean) {
    await repo.toggle_pin_note(note_id, is_pinned);
  }
  ...
  async create_follow_up(input: {
    ...
    if (input.assigned_to_user_id) {
      void notification_service.notify({
        ...
        reference_type: input.user_id ? "user_id" : "order_id",
        reference_id: input.user_id ?? input.order_id ?? "",
      });
    }

    void audit_service.log({
      actor_user_id: input.created_by_user_id,
      action: "customer.follow_up.create",
      resource_type: "follow_up_id",
      resource_id: id,
    });
  ...
  async complete_follow_up(input: { id: string; result_notes?: string; completed_by_user_id: string }) {
    ...
    if (!follow_up) throw_error(OPERATIONS_ERROR.FOLLOW_UP_NOT_FOUND);

    await repo.update_follow_up(input.id, {
      status: "completed",
  ...
  async get_follow_up(id: string) {
    const fu = await repo.get_follow_up(id);
    if (!fu) throw_error(OPERATIONS_ERROR.FOLLOW_UP_NOT_FOUND);
    return fu;
  }

  async list_my_follow_ups(assigned_to_user_id: string, status?: string, page = 1, limit = 20) {
    return repo.list_follow_ups(assigned_to_user_id, status, page, limit);
  }

  async get_follow_ups_by_user(user_id: string) {
    return repo.get_follow_ups_by_user(user_id);
  }
  ...
  async create_case(input: {
    ...
    void audit_service.log({
      actor_user_id: input.created_by_user_id,
      action: "customer.support_case.create",
  ...
  async assign_case(input: { case_id: string; assigned_to_user_id: string; assigned_by_user_id: string }) {
    const c = await repo.get_case(input.case_id);
    if (!c) throw_error(OPERATIONS_ERROR.CASE_NOT_FOUND);

    await repo.update_case(input.case_id, {
  ...
  async reopen_case(input: { case_id: string; reason: string; reopened_by_user_id: string }) {
    const c = await repo.get_case(input.case_id);
    if (!c) throw_error(OPERATIONS_ERROR.CASE_NOT_FOUND);
    if (c.status !== "resolved" && c.status !== "closed") throw_error(OPERATIONS_ERROR.INVALID_STATUS);

    await repo.update_case(input.case_id, {
      status: "reopened",
      reopened_count: (c.reopened_count ?? 0) + 1,
    });
  }

  async close_case(input: { case_id: string; closed_by_user_id: string }) {
    await repo.update_case(input.case_id, { status: "closed" });
  }

  async get_case(id: string) {
    const c = await repo.get_case(id);
    if (!c) throw_error(OPERATIONS_ERROR.CASE_NOT_FOUND);
    return c;
  }

  async list_cases(page = 1, limit = 20, status?: string, assigned_to?: string) {
    return repo.list_cases(page, limit, status, assigned_to);
  }

  async get_cases_by_user(user_id: string) {
    return repo.get_cases_by_user(user_id);
  }
  ...
  async add_case_message(input: { case_id: string; message: string; is_internal?: boolean; author_user_id: string }) {
    ...
    const id = await repo.insert_message({
      id: generate_id(),
    ...
    if (!input.is_internal) {
      await repo.update_case(input.case_id, { status: "in_progress" });
    }

    return id;
  }

  async get_case_messages(case_id: string) {
    return repo.get_messages(case_id);
  }
}
