...
export class ProductWorkflowService {
  async log_change(input: {
    ...
    void audit_service.log({
      actor_user_id: input.changed_by_user_id,
      action: "product.change.log",
      resource_type: "product_id",
      resource_id: input.product_id,
      metadata: { change_type: input.change_type, field_name: input.field_name },
    });
  ...
  async get_change_history(product_id: string, change_type?: string) {
    const where = change_type
      ? and(eq(product_change_log.product_id, product_id), eq(product_change_log.change_type, change_type))
      : eq(product_change_log.product_id, product_id);
    return db.select().from(product_change_log).where(where).orderBy(desc(product_change_log.created_at));
  }

  async schedule_publishing(input: {
    product_id: string;
    ...
    const [created] = await db
      .insert(product_publishing_schedule)
      .values({
        id: generate_id(),
        product_id: input.product_id,
        action: input.action,
    ...
    void audit_service.log({
      ...
      action: "product.publishing.schedule",
      resource_type: "product_id",
      resource_id: input.product_id,
      metadata: { scheduled_action: input.action, scheduled_at: input.scheduled_at },
    });

    return db
      .select()
      .from(product_publishing_schedule)
      .where(eq(product_publishing_schedule.id, created.id))
      .then((r) => r[0] ?? null);
  }

  async cancel_schedule(input: { schedule_id: string; cancelled_by_user_id: string; reason?: string }) {
    await db
      .update(product_publishing_schedule)
      .set({
        status: "cancelled",
        cancelled_by_user_id: input.cancelled_by_user_id,
        cancel_reason: input.reason ?? null,
      })
      .where(eq(product_publishing_schedule.id, input.schedule_id));
  }
  ...
  async get_scheduled_actions(product_id?: string) {
    if (product_id) {
      return db
        .select()
        .from(product_publishing_schedule)
        .where(and(eq(product_publishing_schedule.product_id, product_id), eq(product_publishing_schedule.status, "pending")))
        .orderBy(product_publishing_schedule.scheduled_at);
    }
    return db
      .select()
      .from(product_publishing_schedule)
      .where(eq(product_publishing_schedule.status, "pending"))
      .orderBy(product_publishing_schedule.scheduled_at);
  }
  ...
  async execute_pending_schedules() {
    const pending = await db
      ...
      .from(product_publishing_schedule)
      .where(
        and(eq(product_publishing_schedule.status, "pending"), product_publishing_schedule.scheduled_at, product_publishing_schedule.scheduled_at),
      );
    ...
    for (const schedule of pending) {
      ...
      try {
        ...
        await db
          .update(product_publishing_schedule)
          .set({ status: "executed", executed_at: new Date().toISOString() })
          .where(eq(product_publishing_schedule.id, schedule.id));
      } catch {
        await db
          .update(product_publishing_schedule)
          .set({ status: "failed" })
