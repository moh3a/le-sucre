...
export class PaymentOperationsService {
  // ─── MANUAL VERIFICATION ──────────────────────
  async create_verification(input: {
    order_id: string;
    amount: number;
    currency?: string;
    ...
    void audit_service.log({
      ...
      action: "payment.verification.create",
    ...
    return db
      ...
      .where(eq(payment_verifications.id, created.id))
      .then((r) => r[0] ?? null);
  }

  async verify_payment(input: {
    id: string;
    ...
    const verification = await db
      .select()
      .from(payment_verifications)
      .where(eq(payment_verifications.id, input.id))
      .limit(1)
      .then((r) => r[0] ?? null);
    ...
    if (input.status === "verified") {
      await order_repository.update_order_payment(verification.order_id, {
        payment_status: "paid",
    ...
    void audit_service.log({
      actor_user_id: input.verified_by_user_id,
      action: "payment.verification.review",
      resource_type: "payment_verification_id",
  ...
  async count_pending() {
    const [{ count: total }] = await db
      .select({ count: count() })
      .from(payment_verifications)
      .where(eq(payment_verifications.status, "pending"));
    return Number(total ?? 0);
  }

  // ─── REFUND REQUESTS ──────────────────────────
  async request_refund(input: {
    order_id: string;
    amount: number;
    reason: string;
  ...
  async approve_refund(input: {
    ...
    await db
      ...
      .where(eq(refund_requests.id, input.id));

    void audit_service.log({
      actor_user_id: input.approved_by_user_id,
      action: "payment.refund.approve",
      resource_type: "refund_request_id",
      resource_id: input.id,
    });
  }

  async process_refund(input: {
    id: string;
    ...
    if (input.status !== "failed") {
      await order_repository.update_order_payment(refund.order_id, { payment_status: "refunded" });
    }

    void audit_service.log({
      actor_user_id: input.processed_by_user_id,
      action: "payment.refund.process",
      resource_type: "refund_request_id",
      resource_id: input.id,
      metadata: { status: input.status },
    });
  ...
  // ─── PARTIAL PAYMENTS ─────────────────────────
  async record_partial_payment(input: {
    order_id: string;
