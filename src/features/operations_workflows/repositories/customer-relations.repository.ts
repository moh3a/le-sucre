...
export class CustomerRelationsRepository {
  ...
  async get_contacts_by_user(user_id: string, page = 1, limit = 20) {
    ...
    const [items, [{ total }]] = await Promise.all([
      db
        ...
        .where(eq(customer_contacts.user_id, user_id))
        .orderBy(desc(customer_contacts.created_at))
        .limit(limit)
        .offset(offset),
  ...
  async get_contacts_by_order(order_id: string) {
    return db
      .select()
      .from(customer_contacts)
      .where(eq(customer_contacts.order_id, order_id))
      .orderBy(desc(customer_contacts.created_at));
  }
  ...
  async get_notes_by_user(user_id: string, note_type?: string) {
    const where = note_type
      ? and(eq(customer_notes.user_id, user_id), eq(customer_notes.note_type, note_type))
      : eq(customer_notes.user_id, user_id);
  ...
  async list_follow_ups(assigned_to_user_id: string, status?: string, page = 1, limit = 20) {
    const offset = (page - 1) * limit;
    const where = status
      ? and(eq(customer_follow_ups.assigned_to_user_id, assigned_to_user_id), eq(customer_follow_ups.status, status))
      : eq(customer_follow_ups.assigned_to_user_id, assigned_to_user_id);
    const [items, [{ total }]] = await Promise.all([
  ...
  async get_follow_ups_by_user(user_id: string) {
    return db
      ...
      .where(eq(customer_follow_ups.user_id, user_id))
      .orderBy(desc(customer_follow_ups.scheduled_at));
  }
  ...
  async get_overdue_follow_ups() {
    return db
      ...
      .where(
        and(
          eq(customer_follow_ups.status, "pending"),
          sql`${customer_follow_ups.scheduled_at} < NOW()`,
        ),
      )
      .orderBy(desc(customer_follow_ups.scheduled_at));
  }
