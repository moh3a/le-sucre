...
export class DeliveryRepository {
  ...
  async count_failed_deliveries(since?: string) {
    const where = since
      ? and(
          ...
          sql`${delivery_attempts.attempted_at} >= ${since}`,
        )
      : eq(delivery_attempts.status, "failed");
  ...
  async list_attempts(page = 1, limit = 20, status?: string, delivery_person_id?: string) {
    ...
    if (delivery_person_id)
      conditions.push(eq(delivery_attempts.delivery_person_id, delivery_person_id));
  ...
  async get_stats() {
    const [[{ failed }], [{ successful }], [{ pending }], [{ rto }], [{ today_failed }]] =
      await Promise.all([
        db
          .select({ failed: count() })
          .from(delivery_attempts)
          .where(eq(delivery_attempts.status, "failed")),
        db
          .select({ successful: count() })
          .from(delivery_attempts)
          .where(eq(delivery_attempts.status, "successful")),
        db
          .select({ pending: count() })
          .from(delivery_attempts)
          .where(
            sql`${delivery_attempts.next_attempt_at} IS NOT NULL AND ${delivery_attempts.next_attempt_at} >= NOW()`,
          ),
        db
          .select({ rto: count() })
          .from(delivery_attempts)
          .where(eq(delivery_attempts.status, "cancelled")),
        db
          .select({ today_failed: count() })
          .from(delivery_attempts)
          .where(
            and(
              eq(delivery_attempts.status, "failed"),
