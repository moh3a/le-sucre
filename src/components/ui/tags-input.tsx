...
function TagsInputList({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "border-input bg-input/30 focus-within:ring-ring flex min-h-10 w-full flex-wrap items-center gap-1.5 rounded-full border px-3 py-2 text-sm focus-within:ring-1 disabled:cursor-not-allowed disabled:opacity-50",
        className,
      )}
      {...props}
    />
...
function TagsInputItem({
  ...
  return (
    <TagsInputPrimitive.Item
      ...
      className={cn(
        "rounded-full data-editing:ring-ring [&[data-highlighted]:not([data-editing])]:bg-accent [&[data-highlighted]:not([data-editing])]:text-accent-foreground inline-flex max-w-[calc(100%-8px)] items-center gap-1.5 border bg-background px-2.5 py-1 text-sm focus:outline-hidden data-disabled:cursor-not-allowed data-disabled:opacity-50 data-editable:select-none data-editing:bg-transparent data-editing:ring-1 [&:not([data-editing])]:pr-1.5",
        className,
      )}
      {...props}
    >
      <TagsInputPrimitive.ItemText className="truncate">{children}</TagsInputPrimitive.ItemText>
      <TagsInputPrimitive.ItemDelete className="ring-offset-background size-4 shrink-0 rounded-full opacity-70 transition-opacity hover:opacity-100">
