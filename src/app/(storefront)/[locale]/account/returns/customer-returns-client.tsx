      ...
      <div className="space-y-4">
        {(orderId ? filtered : requests)?.map((r: any) => (
          <div key={r.id} className="rounded-lg border bg-white p-4">
            ...
            <p className="text-xs text-gray-500">{t("order", { id: r.order_id?.slice(0, 12) ?? "—" })}</p>
            {r.reason && <p className="text-xs text-gray-500 mt-1">{t("reason", { reason: r.reason })}</p>}
            {r.created_at && <p className="text-xs text-gray-400 mt-1">{t("submitted", { date: new Date(r.created_at).toLocaleDateString() })}</p>}
          </div>
        ))}
        {(!(orderId ? filtered : requests) || (orderId ? filtered : requests)?.length === 0) && (
          <p className="text-center text-sm text-gray-400 py-8">{t("no_returns")}</p>
        )}
      </div>

      <p className="text-xs text-gray-400 text-center">
        {t.rich("create_return_text", {
          link: (chunks) => <Link href="/account/orders" className="text-blue-600 hover:underline">{chunks}</Link>,
        })}
      </p>
    </div>
  );
